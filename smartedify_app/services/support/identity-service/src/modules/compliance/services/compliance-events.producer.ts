...
export class ComplianceEventsProducer implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit(): Promise<void> {
    if (this.kafkaDisabled) {
      this.logger.warn('Kafka producer initialisation skipped because KAFKA_DISABLED=true');
      return;
    }
  ...
  async emitDataExportRequested(payload: ComplianceJobEventPayload): Promise<void> {
    await this.emitEvent('DataExportRequested', payload);
  }

  async emitDataDeletionRequested(payload: ComplianceJobEventPayload): Promise<void> {
    await this.emitEvent('DataDeletionRequested', payload);
  }

  private async emitEvent(topic: string, payload: ComplianceJobEventPayload): Promise<void> {
    if (this.kafkaDisabled) {
      this.logger.debug(`Kafka disabled, skipping emit for ${topic}`);
      return;
    }

    this.logger.debug(`Publishing ${topic} with payload ${JSON.stringify(payload)}`);
