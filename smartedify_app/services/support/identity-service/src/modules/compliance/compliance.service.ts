...
export class ComplianceService {
  ...
  async handleJobCallback(jobId: string, callback: ComplianceJobCallbackDto): Promise<ComplianceJob> {
  ...
  private async createJob(
    ...
    if (!services.length) {
      throw new BadRequestException('At least one affected service is required');
    }
  ...
  private async publishEvent(type: ComplianceJobType, job: ComplianceJob): Promise<void> {
  ...
  private async registerServices(job: ComplianceJob, services: string[]): Promise<void> {
  ...
  private async triggerInternalCoordinations(job: ComplianceJob): Promise<void> {
  ...
  private async refreshJobStatus(jobId: string): Promise<ComplianceJob> {
    ...
    if (statuses.every((status) => status === ComplianceJobServiceStatus.COMPLETED)) {
      job.status = ComplianceJobStatus.COMPLETED;
      job.completed_at = job.completed_at ?? new Date();
    } else if (statuses.some((status) => status === ComplianceJobServiceStatus.FAILED)) {
  ...
  private normaliseServices(services: string[]): string[] {
    return Array.from(new Set((services ?? []).map((service) => service.trim()))).filter(
      Boolean,
    );
  }
