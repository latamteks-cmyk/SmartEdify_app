...
export class QrcodesService {
  ...
  async generateQrCode(payload: any): Promise<string> {
    ...
    const jwsResult = await jose.JWS.createSign(options, key).update(JSON.stringify(payload)).final();
  ...
  async validateQrCode(token: string): Promise<any> {
    try {
      ...
      if ((verified.protected as any).kid !== kid) { // Explicit cast for type inference workaround
