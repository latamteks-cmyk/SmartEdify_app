...
// Define a type for the request object to avoid 'any'
...
export class ClientAuthGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    const req: ClientAuthRequest = context.switchToHttp().getRequest();
    ...
    try {
      ...
      if (!kid || !clientId) {
        throw new UnauthorizedException(
          'Missing kid or iss in client assertion',
        );
      }
      ...
      if (verifiedPayload.aud !== tokenEndpointUrl) {
        throw new UnauthorizedException(
          'Invalid audience for client assertion',
        );
      }

      // The issuer and subject must be the client_id
      if (
        verifiedPayload.iss !== clientId ||
        verifiedPayload.sub !== clientId
      ) {
        throw new UnauthorizedException(
          'Invalid issuer or subject for client assertion',
        );
      }
      ...
      throw new UnauthorizedException(
        `Client assertion validation failed: ${error instanceof Error ? error.message : 'Unknown error'}`,
      );
    }
  }
}
