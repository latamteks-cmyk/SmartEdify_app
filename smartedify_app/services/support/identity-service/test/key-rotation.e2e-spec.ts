...
describe('Key Rotation (e2e)', () => {
  ...
  describe('Key Rotation Flow', () => {
    it(
      'should correctly rotate and expire keys',
      async () => {
        const oldKeyData = await TestDataBuilder.createOldActiveKey(
          tenantId,
          91,
        );
        const savedOldKey = await signingKeyRepository.save(oldKeyData);

        const initialKeys = await signingKeyRepository.find({
        ...
        const keysAfterExpiration = await signingKeyRepository.find({
          where: { tenant_id: tenantId },
        });

        expect(
          keysAfterExpiration.find((k) => k.status === KeyStatus.EXPIRED),
        ).toBeDefined();
        expect(
          keysAfterExpiration.find((k) => k.status === KeyStatus.ROLLED_OVER),
        ).toBeUndefined();
        expect(
          keysAfterExpiration.find((k) => k.status === KeyStatus.ACTIVE),
        ).toBeDefined();
  ...
  describe('Edge Cases', () => {
    it('should handle no keys to rotate', async () => {
      ...
      const keys = await signingKeyRepository.find({
        where: { tenant_id: tenantId },
      });
    ...
    it('should handle keys that are not old enough to rotate', async () => {
      const newKeyData = await TestDataBuilder.createSigningKey({
        tenantId,
        daysOld: 30,
      });
