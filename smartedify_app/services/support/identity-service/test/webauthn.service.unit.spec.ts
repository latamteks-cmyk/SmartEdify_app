...
describe('WebauthnService (unit)', () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: 'RpService', useValue: { getRpId: () => 'localhost', getExpectedOrigin: () => 'http://localhost:3000' } },
        { provide: 'UsersService', useValue: { findByEmail: async () => ({ id: 'user-id', email: 'test@test.com' }) } },
      ],
    }).compile();
    service = module.get<WebauthnService>(WebauthnService);
    repo = module.get<Repository<WebAuthnCredential>>(getRepositoryToken(WebAuthnCredential));
  });
  ...
  it('should persist advanced fields from registrationInfo', async () => {
    ...
    // @ts-ignore
    await service['persistCredential'](registrationInfo, response, { id: 'user-id', email: 'test@test.com' });
